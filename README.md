...
## Pictures
Current state as of 07.24:

![Fuselage](docs/fuselage0724.png)

![V-tail mount](docs/vtail0724.png)

![Wing preview](docs/wtip0724.png)

![LoRa](docs/loramodule.jpg)
...
the nose ring, and the wing saddle, which print in other materials for stiffness.
The two V-tail panels are NACA 0009, single-wall, solid only at the root tongue, and the motor firewall prints in ASA or PA6-CF at high density with the C2836 bolt circle and a built-in 2° down / 2° right thrust offset.
For early flights, treat the winglets as optional swap-ons.


Assemble from the spar outward. Thread the carbon spar setup through all wing modules, epoxy them to each other and to the spar.
Glue the fuselage segments at their sleeves and use acetone to bond them together chemically
Print the firewall with the nose, but with two separate materials, and bolt the motor down over washers.
...
Finish with balance and the maiden flight. Slide the battery until the CG sits about 30% of the chord back from the leading edge, erring nose-heavy.
